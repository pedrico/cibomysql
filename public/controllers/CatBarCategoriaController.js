...
myApp.controller('AppCtrl', ['$scope', '$http', 'Upload', '$window', function ($scope, $http, Upload, $window) {
    ...
    var refresh = function () {
        $http.get('CatBarCategoria/cocina').then(function (response) {
    ...
    $scope.addContact = function () {
        ...
        if (vm.upload_form.file.$valid && vm.file) { //check if from is valid
            ...
            subir(vm.file, function () {
                ...
                $http.post('CatBarCategoria/cocina', $scope.cocina).then(function (response) {
            ...
            $http.post('CatBarCategoria/cocinaNoImagen', $scope.cocina).then(function (response) {
                console.log(response);
                refresh();
            });
        }
    ...
    $scope.remove = function (id) {
        console.log(id);
        $http.delete('CatBarCategoria/cocina/' + id).then(function (response) {
            refresh();
        });
    ...
    $scope.edit = function (id) {
        console.log(id);
        $http.get('CatBarCategoria/cocina/' + id).then(function (response) {
            console.log(response);
    ...
    $scope.redireccionar = function (id) {
        console.log(id + "--hola");
        $http.get('CatBarCategoria/RedirectIngreCocina').then(function (data, status) {
    ...
    $scope.update = function () {
        if (vm.upload_form.file.$valid && vm.file) { //check if from is valid  
            ...
            subir(vm.file, function () {
                ...
                $http.put('CatBarCategoria/cocina/' + $scope.cocina._id, $scope.cocina).then(function (response) {
                    $scope.ShowAgregar = true;
                    refresh();
                });
            }); //call upload function            
            ...
            console.log(vm.file);
            console.log($scope.cocina);
            $http.put('CatBarCategoria/cocinaNoImagen/' + $scope.cocina._id, $scope.cocina).then(function (response) {
                $scope.ShowAgregar = true;
                refresh();
            });
        }

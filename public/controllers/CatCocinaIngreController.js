...
myApp.controller('CtrlIngrediente', ['$scope', '$http','Upload','$window', function ($scope, $http,Upload,$window) {
    ...
    var refresh = function () {
        $http.get('CatCocinaIngre/ingrediente').then(function (response) {
    ...
    $scope.addContact = function () {
        ...
        if (vm.upload_form.file.$valid && vm.file) { //check if from is valid
            ...
            subir(vm.file, function(){
                ...
                $http.post('CatCocinaIngre/ingrediente', $scope.cocina).then(function (response) {
            ...
            $http.post('CatCocinaIngre/ingredienteNoImagen', $scope.cocina).then(function (response) {
                console.log(response);
                refresh();
            });
        }
    ...
    $scope.remove = function(id){
        console.log(id);
        $http.delete('CatCocinaIngre/ingrediente/'+ id).then(function(response){
            refresh();
        }) ;
    ...
    $scope.edit = function(id){
        console.log(id);
        $http.get('CatCocinaIngre/ingrediente/'+ id).then(function(response){
            console.log(response);
    ...
    $scope.update = function(){
        if (vm.upload_form.file.$valid && vm.file) { //check if from is valid  
            ...
            subir(vm.file, function(){
                ...
                $http.put('CatCocinaIngre/ingrediente/'+ $scope.cocina._id, $scope.cocina).then(function(response){
                    $scope.ShowAgregar = true;
                    refresh();
                });        
            }); //call upload function            
            ...
            console.log(vm.file);
            $http.put('CatCocinaIngre/ingredienteNoImagen/'+ $scope.cocina._id, $scope.cocina).then(function(response){
                $scope.ShowAgregar = true;
                refresh();
            }); 
        }        

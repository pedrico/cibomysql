            ...
            con.query(sql, [values], function (err, result) {
                ...
                con.query(sql, [values], function (err, result) {
                    ...
                    con.query(sql, function (err, result, fields) {
                        ...
                        values = [
                            [NumeroMesa, 1, idPlato, 2, result[0].precio, 1, 0, new Date().addHours(-6).toISOString().slice(0, 19).replace('T', ' '), new Date().addHours(-6).toISOString().slice(0, 19).replace('T', ' ')]
                        ];
                        con.query(sql, [values], function (err, result) {
                            if (err) throw err;
            ...
            con.query(sql, function (err, result, fields) {
                ...
                if (sesionCerrada == 0) {
                    ...
                    con.query(sql, function (err, result, fields) {
                        ...
                        sql = "INSERT INTO DetalleOrdenMesa (idMesa, numSesion, idItem, Categoria, Precio, Cantidad, Enviada, fechaInsert, fechaUpdate) VALUES ?";
                        values = [
                            [NumeroMesa, NumeroSesion, idPlato, 2, result[0].precio, 1, 0, new Date().addHours(-6).toISOString().slice(0, 19).replace('T', ' '), new Date().addHours(-6).toISOString().slice(0, 19).replace('T', ' ')]
                        ];
                        con.query(sql, [values], function (err, result) {
                            if (err) throw err;
                            console.log("Correlativo");
